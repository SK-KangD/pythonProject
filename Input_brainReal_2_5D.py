...
def read_2dSKK(iname_queue):
    ...
    result.input7= tf.reshape(tf.slice(reading_bytes, [120 * 168 * 3 * 2 + 120 * 120 * 3 * 4], [120 * 120 * 3]), [3, 120, 120])
...
def _generate_image_and_label_batch(xlr1, xlr2, xlr3, xlr4, xlr5, xlr6, xlr7,
    ...
    if shuffle:
        [out_xlr1, out_xlr2, out_xlr3, out_xlr4, out_xlr5, out_xlr6, out_xlr7] = tf.train.shuffle_batch(
            [xlr1, xlr2, xlr3, xlr4, xlr5, xlr6, xlr7],
        ...
        [out_xlr1, out_xlr2, out_xlr3, out_xlr4, out_xlr5, out_xlr6, out_xlr7] = tf.train.batch(
            [xlr1, xlr2, xlr3, xlr4, xlr5, xlr6, xlr7],
            batch_size=batch_size,
            num_threads=num_preprocess_threads,
            capacity=min_queue_examples + 3 * batch_size)

    return out_xlr1, out_xlr2, out_xlr3, out_xlr4, out_xlr5, out_xlr6, out_xlr7
...
def distorted_inputs(data_dir, batch_size):
    ...
    x_input7 = tf.cast(read_input.input7, tf.float32)
    ...
    x_input7 = tf.transpose(x_input7, [1, 2, 0])
    ...
    return _generate_image_and_label_batch(x_input1, x_input2, x_input3, x_input4, x_input5, x_input6, x_input7, min_queue_examples, batch_size, shuffle=True, eval_test=False)
    ###################################################################
