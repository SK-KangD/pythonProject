...
def train(train_dir):
    with tf.Graph().as_default():
        ...
        # Sino, Label, MLEM, MRI
        xlr1, xlr2, xlr3, xlr4, xlr5, xlr6, xlr7 = lays.distorted_inputs()
        ...
        lays._activation_image_2d(tf.nn.relu(xlr6), 0, name='reference_' + '_u')
        ...
        loss2 = tf.reduce_mean(tf.losses.absolute_difference(u, xlr6))
        # loss_tv_u =  1e-7 * tf.reduce_mean(lays.total_variation(u))
        ...
        loss2_p = tf.reduce_mean(tf.losses.absolute_difference(u_prj, xlr2)) / 200

        tf.summary.scalar('u proj loss', loss2_p)

        loss2 = loss2 + 0.01*loss2_p
        ...
        loss1_reg = tf.add_n([tf.nn.l2_loss(v) for v in g_vars
                           if 'bias' not in v.name]) * 0.0001

        loss2_reg = tf.add_n([tf.nn.l2_loss(v) for v in u_vars
                           if 'bias' not in v.name]) * 0.0001

        loss1 = loss1 + loss1_reg
        loss2 = loss2 + loss2_reg
        ...
        saver_re = tf.train.Saver(var_list=t_vars)
        #
        sum_writer = tf.summary.FileWriter(os.path.join(train_dir, 'summs'), graph=tf.get_default_graph())
        summaries = tf.summary.merge_all()
        #
        # ckpt_path = r'D:\SKK_DL\Unfold_MLAA\TrainResults\12_09_06_12_unfold'
